...
|------|------------------|---------------|
| **1** | User ID = date + time (unique) | Auto-increment `Long` via `@GeneratedValue(IDENTITY)` in [`User.java`](src/main/java/com/authentication/AuthProject/model/User.java) |
...
|-------|-------------------|
| [`User.java`](src/main/java/com/authentication/AuthProject/model/User.java) | `@Getter @Setter @NoArgsConstructor` — keep `@PrePersist`/`@PreUpdate` methods |
...
**File:** [`User.java`](src/main/java/com/authentication/AuthProject/model/User.java)
